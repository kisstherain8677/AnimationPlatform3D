...
namespace AnimGenerator
{
    ...
    public class Interpreter : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            //导入这个角色相关的动画、皮肤资源
            //resManager.LoadPackageAssets(packageName);
        }
        ...
        void Update()
        {
            ...
            else if (interpreterMode == EInterpreterMode.Wait)
            {
                ...
                FileInfo[] files = streamingDir.GetFiles("*.json");
        ...
        private IEnumerator Animate(KeyFrame  frame)
        {
            ...
            //animator.SetTrigger(frame.content);
            //Animation anim = go.GetComponent<Animation>();
            //anim.Play(frame.content);
            ...
            gos[frame.name].GetComponent<Animation>().Stop();
        }
        ...
        /// <summary>
        /// 根据一个皮肤预制体所在的路径加载皮肤
        /// 在Resources文件夹中，形式为 playerName/Cn/Skin/partname
        /// </summary>
        private void loadPartSkin(string skinPath,ref GameObject go)
        {
            GameObject goPart= Resources.Load<GameObject>(skinPath);
            goPart = GameObject.Instantiate<GameObject>(goPart);
            goPart.SetActive(false);
            UnityFactory.factory.ReplaceSkin(
                go.GetComponentInChildren<UnityArmatureComponent>().armature,
                goPart.GetComponent<UnityArmatureComponent>().armature.armatureData.defaultSkin
                );
            Destroy(goPart);
        ...
        private void loadAllSkin(ref GameObject go,int suitIndex)
        {
            //换到战斗服装
            loadPartSkin("ChangeTest/C"+suitIndex+"/Skin/body", ref go);

            //换到战斗表情
            loadPartSkin("ChangeTest/C" + suitIndex + "/Skin/face", ref go);
        }
...
//        private GameObject GenerateDrag onbones(GameObject go)
//        {
